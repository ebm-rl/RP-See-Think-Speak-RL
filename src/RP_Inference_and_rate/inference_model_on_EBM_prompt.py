...
SYSTEM_SUFFIX_VANILLA = (
    ...
    "- Directly output the final natural spoken line by the Assistant.\n"
...
def _build_user_prompt(example, mode="ours") -> str:
    ...
    if mode == "ours":
        ...
        instruction = (
            "### INSTRUCTION\n"
            f"Generate the next line for **{assistant_name}**.\n"
            "(1) **Step 1 <think>...</think> (Analytic Mode)**: Analyze visual information and conversation context to explain the logic step by step.\n"
            f"(2) **Step 2 <answer>...</answer>**: Reply to {user_name} in character.\n\n"
            "**Remember to close all tags.\n**"
            "**CRITICAL FORMATTING RULE**: You MUST close your response with </answer>. Do not stop until you write this tag.\n"
        ...
        instruction = (
            ...
            f"Reply to {user_name} in character. Apply the **High-Stakes Personality Rule** (e.g., humor turns to bravery in danger).\n\n"
            "- Directly output the final natural spoken line by the Assistant.\n"
            "- No speaker name. No quotes."
        )
...
def main():
    ...
    parser.add_argument('--shard_id', type=int, default=0, help="Current shard ID (starting from 0)")
    ...
    with open(backup_file, "w", encoding="utf-8") as f_backup:
        ...
        for i, item in tqdm(enumerate(local_data), 
            ...
            try:
                ...
                res_item["raw_output"] = output_text
