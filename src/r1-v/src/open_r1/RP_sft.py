...
def prepare_dataset(example: Dict[str, Any]) -> Dict[str, List[Dict[str, Any]]]:
    """
    SFT 的核心：
    - Input: System Prompt + Video + Roleplay Prompt (通过 _build_roleplay_prompt_text 生成)
    - Label: Process (<vision>+<think>) + Solution (<answer>)
    """
    
    # 1. 构造 Prompt (User Side)
    prompt_text = _build_roleplay_prompt_text(example)
    
    # 2. 构造完整的 Model Response (Label Side)
    # 数据中:
...
def collate_fn(examples: List[Dict[str, Any]]) -> Dict[str, torch.Tensor]:
    ...
    # # === [Debug] 分开查看 Mask 前后 ===
    # # 1. 找分界点 (Training 开始的第一个位置)
    # _idx = (labels[0] != -100).nonzero(as_tuple=True)[0][0].item()  
    # # 2. 打印 [Masked] 部分 (分界线前 10 个)
    # print(f"\n[MASKED (扔掉)]: ... {processor.tokenizer.decode(input_ids[0][_idx-10 : _idx])}") 
    # # 3. 打印 [Train] 部分 (分界线后 10 个)
    # print(f"[TRAIN  (学习)]: >>> {processor.tokenizer.decode(input_ids[0][_idx : _idx+10])} ...\n")
...
class VisualizerCallback(TrainerCallback):
    ...
    def on_evaluate(self, args, state, control, model, **kwargs):
        ...
        for example in samples:
            # 拿到原始 messages
            # example 是经过 prepare_dataset 处理过的，结构是 {"messages": [...]}
            messages = example["messages"]
            
            # 【关键】提取 User 输入，去掉最后的 Assistant 回复 (Ground Truth)
            # 这样模型就看不到答案，被迫自己推理
            ...
            # 生成回复 (Inference)
            # 设置 max_new_tokens=2048 确保 COT 不被截断
            with torch.no_grad():
                generated_ids = model.generate(
                    **inputs, 
                    max_new_tokens=4096,  # <--- 这里改大了，防止截断
                    temperature=0.7,      # 稍微给点随机性，看模型生成能力
                    do_sample=True
                )
        ...
        #     columns = ["Step", "Prompt", "Ground Truth (COT+Ans)", "Model Prediction (COT+Ans)"]
        #     data = [[r["step"], r["input_prompt_snippet"], r["ground_truth_full"], r["model_prediction_full"]] for r in results]
        #     wandb.log({"visual_samples": wandb.Table(columns=columns, data=data)})
...
if __name__ == "__main__":
    ...
    #     raw_dataset =  DatasetDict({"train": Dataset.from_json(script_args.dataset_name)})
    # else:
    #     # Load the dataset
    ...
    if script_args.dataset_name.endswith('.json') or script_args.dataset_name.endswith('.jsonl'):
        # 【修改1】直接读取为 Dataset 对象，去掉 DatasetDict 的包裹
        ...
        # 【修改2】如果是 Hub 上的 DatasetDict，尝试提取 'train' 分支
        if isinstance(raw_val, DatasetDict):
            if "train" in raw_val:
                raw_dataset = raw_val["train"]
            else:
                # 假如没有 train 键，就取第一个键 (防止报错)
                raw_dataset = raw_val[list(raw_val.keys())[0]]
        else:
            raw_dataset = raw_val
        
    # 【现在 raw_dataset 是真正的 Dataset 对象，可以安全切分了】
    dataset_splits = raw_dataset.train_test_split(test_size=0.1, seed=42)
        
    print(f"Train set size: {len(dataset_splits['train'])}")
    print(f"Eval set size: {len(dataset_splits['test'])}")
    ...
    visualizer_callback = VisualizerCallback(
        ...
        processor=processor, # 传入 processor 以便处理视频输入
        num_samples=4
    )
