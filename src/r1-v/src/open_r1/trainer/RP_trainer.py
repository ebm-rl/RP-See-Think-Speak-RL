...
class Qwen2VLGRPOTrainer(Trainer):
    ...
    def _log_comprehensive_metrics(self, inputs, completion_texts, gt_answer, rewards_tensor, problem_ids):
        """
        全面记录 Vision(CLIP), Think(ICLG), Answer(BERT) 及其对应的文本内容
        """
        ...
        log_dir = "/data/wm/Video-R1/src/r1-v/src/open_r1/debug_log/NEW"  # 请确保路径与你之前的保持一致
        ...
        # 2. 准备数据
        # rewards_tensor shape: (Batch * Generations, 4) -> [Acc, Format, Clip, Iclg]
        # 注意：inputs 的长度是 Batch，而 completions 的长度是 Batch * Generations
        # 我们需要根据索引映射回 inputs
        ...
        with open(log_path, "a", encoding="utf-8") as f:
            ...
            for i, content in enumerate(completion_texts):
                ...
                # 提取各项 Reward 值 (注意：这里假设列顺序是 0:Acc, 1:Format, 2:Clip, 3:ICLG)
                # 务必使用 .item() 转换为 python float
                acc_score = rewards_tensor[i, 0].item()
                fmt_score = rewards_tensor[i, 1].item()
                clip_score = rewards_tensor[i, 2].item()
                iclg_score = rewards_tensor[i, 3].item()
                ...
                # vision_text = v_match.group(1).strip()[:300] + "..." if v_match else "[No Vision]" # 截断一下防止太长
                # think_text = t_match.group(1).strip()[:300] + "..." if t_match else "[No Think]"
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        # 调用日志函数
        # 注意：你需要确保在这里能访问到 completion_texts (通常在 compute_loss 开始时解包出来的)
        # gt_answer 也在你之前的代码片段中获取了
        try:
            self._log_comprehensive_metrics(
                inputs=inputs, 
                completion_texts=completion_texts, # 确保这是 decode 后的纯文本列表
                gt_answer=gt_answer, 
                rewards_tensor=rewards_per_func,   # 此时包含了所有4个reward
                problem_ids=batch_problem_ids
            )
        except Exception as e:
            print(f"Logging Error: {e}") # 防止日志报错中断训练
        # ==================== 【新增代码位置 END】 ======================
